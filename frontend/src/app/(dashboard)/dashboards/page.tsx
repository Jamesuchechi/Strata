...
export default function DashboardsPage() {
  ...
  const [widgets, setWidgets] = useState<DashboardWidget[]>([
    {
      ...
      metricValue: "25 Records",
      metricSubtext: "100% Valid / Clean",
    },
    {
      id: "w2",
      type: "metric",
      title: "Average Customer Charge",
      metricValue: "$82.40 / mo",
      metricSubtext: "+4.2% vs previous period",
    },
    {
      id: "w3",
      type: "chart",
      title: "Revenue by Segment Breakdown",
      chartType: "bar",
      colX: "country",
      colY: "monthly_charges",
    },
    {
      id: "w4",
      type: "note",
      ...
      noteContent:
        "High-retention customer segments show an average monthly spend of $82+. Customer churn is concentrated in lower-tenure trial cohorts.",
    },
  ]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setDatasets(list);
        if (list.length > 0) {
          const preview = await fetchDatasetPreview(list[0].id);
  ...
  const addWidget = () => {
    const newW: DashboardWidget = {
      id: `widget_${Date.now()}`,
      type: newWidgetType,
      title: newWidgetTitle || "New Dashboard Card",
      metricValue: newMetricVal || "98.5%",
      metricSubtext: "Live data feed",
